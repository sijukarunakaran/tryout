...
public struct Reducer<State: Sendable, Action: Sendable> {
    ...
    /// Enable calling the struct like a function. Always called on the main actor via `Store`.
    @MainActor
    ...
    public func scope<GlobalState, GlobalAction>(
        ...
        action casePath: CasePath<GlobalAction, Action>
    ) -> Reducer<GlobalState, GlobalAction> {
        Reducer<GlobalState, GlobalAction> { globalState, globalAction in
            guard let localAction = casePath.extract(globalAction) else { return .none }
